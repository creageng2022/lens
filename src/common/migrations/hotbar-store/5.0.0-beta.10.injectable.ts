...
import { defaultHotbarCells, getEmptyHotbar, HotbarItem } from "../../hotbar-store/hotbar-types";
import { catalogEntity } from "../../../main/catalog-sources/general";
import { MigrationDeclaration, migrationLog } from "../helpers";
import { generateNewIdFor } from "../utils";
import type { HotbarStoreModel } from "../../hotbar-store/store";
...
function getMirgation({ userDataPath }: Dependencies): MigrationDeclaration {
  return {
    version: "5.0.0-beta.10",
    run(store) {
      const rawHotbars = store.get("hotbars");
      const hotbars: HotbarStoreModel["hotbars"] = Array.isArray(rawHotbars) ? rawHotbars.filter(h => h && typeof h === "object") : [];
      ...
      try {
        ...
        for (const hotbar of workspaceHotbars.values()) {
          ...
          hotbars.push(hotbar as HotbarStoreModel["hotbars"][number]);
        }
