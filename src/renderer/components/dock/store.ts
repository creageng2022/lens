...
export class DockStore implements DockStorageState {
  ...
  createTab = action((rawTabDesc: DockTabCreate, addNumber = true): DockTabData => {
    const {
      id = uuid.v4(),
      kind,
      pinned = false,
  ...
  private closeTabs(tabs: DockTabData[]) {
  ...
  renameTab = action((tabId: TabId, title: string) => {
    this.getTabById(tabId).title = title;
  });
